#!/usr/bin/env python3
"""
MDBX Benchmark Log Parser
Parses MDBX benchmark logs and generates a structured CSV output.
"""
...
def parse_data_preparation(log_content: str) -> Dict:
    ...
    # Find commit time stats
    commit_pattern = r'Total commit time: (\d+) ms'
    commit_match = re.search(commit_pattern, log_content)
    if commit_match:
        data_prep['total_commit_time_ms'] = int(commit_match.group(1))
    
    avg_commit_pattern = r'Average commit time per batch: ([\d.]+) ms'
    avg_commit_match = re.search(avg_commit_pattern, log_content)
    if avg_commit_match:
        data_prep['avg_commit_time_per_batch_ms'] = float(avg_commit_match.group(1))
...
def main():
    ...
    # Define CSV headers with readable names
    headers = [
        'test_type',
        'round',
        'total_time_ms',
    ...
    # Print summary
    print(f"Parsed MDBX benchmark log: {log_file}")
