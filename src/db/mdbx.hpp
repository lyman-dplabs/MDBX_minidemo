...
// Custom ByteView type (equivalent to silkworm's ByteView)
...
constexpr size_t operator""_Tebi(unsigned long long value) {
    return value * (1ULL << 40);
}

constexpr size_t operator""_Gibi(unsigned long long value) {
    return value * (1ULL << 30);
}

constexpr size_t operator""_Mebi(unsigned long long value) {
    return value * (1ULL << 20);
}

constexpr size_t operator""_Kibi(unsigned long long value) {
    return value * (1ULL << 10);
}

namespace datastore::kvdb {
...
using WalkFuncRef = function_ref<void(ByteView key, ByteView value)>;
...
inline ByteView from_slice(const Slice slice) {
    return {static_cast<const std::byte*>(slice.data()), slice.length()};
}
