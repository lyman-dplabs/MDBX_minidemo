...
static CursorResult strict_lower_bound(ROCursor& cursor, const ByteView key) {
    ...
    if (!cursor.lower_bound(key_slice, /*throw_notfound=*/false)) {
...
size_t cursor_for_prefix(ROCursor& cursor, const ByteView prefix, WalkFuncRef walker,
    ...
    size_t ret{0};
    Slice prefix_slice{prefix.data(), prefix.size()};
    auto data{cursor.lower_bound(prefix_slice, false)};
    while (data) {
        if (!data.key.starts_with(prefix_slice)) {
            break;
        }
        ++ret;
        walker(from_slice(data.key), from_slice(data.value));
...
size_t cursor_erase_prefix(RWCursor& cursor, const ByteView prefix) {
    size_t ret{0};
    Slice prefix_slice{prefix.data(), prefix.size()};
    auto data{cursor.lower_bound(prefix_slice, /*throw_notfound=*/false)};
    while (data) {
        if (!data.key.starts_with(prefix_slice)) {
            break;
        }
...
size_t cursor_erase(RWCursor& cursor, const ByteView set_key, const CursorMoveDirection direction) {
    Slice set_key_slice{set_key.data(), set_key.size()};
    CursorResult data{
        direction == CursorMoveDirection::kForward
            ? cursor.lower_bound(set_key_slice, /*throw_notfound=*/false)
