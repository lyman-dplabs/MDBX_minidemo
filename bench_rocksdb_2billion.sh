...
LOG_FILE="${LOGS_DIR}/rocksdb_bench_2billion_${TIMESTAMP}.log"
SUMMARY_FILE="${LOGS_DIR}/rocksdb_bench_2billion_${TIMESTAMP}_summary.txt"
...
show_help() {
    echo -e "${BOLD}RocksDB 20亿KV基准测试脚本${NC}"
    echo
    ...
    echo -e "    $0 --config configs/rocksdb_high_throughput.json"
    echo
    echo -e "    # Run with both configs"
    echo -e "    $0 --config configs/rocksdb_high_throughput.json --bench-config configs/bench_2billion.json"
    echo
}
...
run_benchmark() {
    log_step "Running RocksDB 20亿KV基准测试"
    ...
    if [[ -z "${CONFIG_FILE}" ]]; then
        CONFIG_FILE="configs/rocksdb_high_throughput.json"
    fi
    ...
    cd "${BUILD_DIR}"
    if ./rocksdb_bench "${args[@]}" 2>&1 | tee -a "${LOG_FILE}"; then
        echo | tee -a "${LOG_FILE}"
...
print_banner() {
    echo
    echo -e "${PURPLE}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${PURPLE}║${NC}              ${BOLD}RocksDB 20亿KV基准测试${NC}                  ${PURPLE}║${NC}"
